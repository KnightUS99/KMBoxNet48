...
namespace KMBoxNet48
{
    ...
    public class KmBoxClient
    {
        ...
        public async Task<bool> Connect()
        {
            ...
            var response = await SendAndReceive_V2<CmdHeadT, CmdHeadT>(request);
        ...
        public async Task<bool> TypeText(string text, int delay = 80)
        {
            ...
            TimeSpan? delayTimeSpan = delay != 0 ? TimeSpan.FromMilliseconds(delay) : (TimeSpan?)null;

            foreach (var character in text)
            {
                (KeyboardButton, KeyboardModifiers) mapping = default;
                if (!KeyboardButtonMap.CharToKeyboardButton.TryGetValue(character, out mapping))
                {
                ...
                if (!await KeyboardButtonDown(mapping.Item1, mapping.Item2))
                    return false;
        ...
        public async Task<bool> SetConfig(IPAddress newIpAddress, ushort newPort)
        {
            ...
            request.rand = BitConverter.ToUInt32(addressAsBytes, 0);
        ...
        {
            if (_latestReportListener != null && !_latestReportListener.Stopped)
            {
        ...
        private async Task<TResponse> SendAndReceive_V2<THead, TResponse>(THead request)
            where THead : struct
            where TResponse : struct
        {
            ...
            // Convert the byte array to the response struct
        ...
        private async Task<TResponse> SendAndRecieve<THead, TRequest, TResponse>(THead head, TRequest request)
            ...
            where TResponse : struct
        {
            // Serialize both 'head' and 'request' structs into a single byte array
            var serialized = StructHelper.StructToByteArray(head, request);

            // Send the byte array using SendAsync, ensuring to pass both the byte array and its length
            await _udpClient.SendAsync(serialized, serialized.Length);

            // Receive the response asynchronously
            var response = await _udpClient.ReceiveAsync();

            // Deserialize the byte array into the response struct
            return StructHelper.ByteArrayToStruct<TResponse>(response.Buffer);
        }
        ...
        private CmdHeadT NextCmdHead(KmCommand command)
        {
            ...
            return new CmdHeadT
            {
                ...
                rand = (uint)rnd.Next(0,int.MaxValue),
